"""`Templates(engine="jinja")`: templates read as SQLAKit 0.20 read them, until 0.22."""
...
pytestmark = pytest.mark.filterwarnings(
    "ignore:`Templates\\(engine='jinja'\\)` is deprecated:DeprecationWarning"
)
...
def db(templates: Path) -> Iterator[Database]:
    db = Database(
        ...
        templates=Templates(templates, engine="jinja"),
    )
...
def test_a_jinja_template_says_it_is_going() -> None:
    with pytest.warns(DeprecationWarning, match="goes in SQLAKit 0.22"):
        Templates("app/sql", engine="jinja")


def test_a_jinja_template_is_checked_as_jinja(templates: Path) -> None:
    (templates / "broken.sql").write_text("SELECT {% if %}")
    with pytest.raises(jinja2.TemplateSyntaxError):
        Templates(templates, engine="jinja").check()
...
def test_the_options_of_one_engine_are_refused_by_the_other() -> None:
    ...
    with pytest.raises(ValueError, match="are for `engine='tpl'`"):
        Templates("app/sql", engine="jinja", macros=[mine])
    with pytest.raises(ValueError, match="are for `engine='tpl'`"):
        Templates("app/sql", engine="jinja", namespace="q")
    with pytest.raises(ValueError, match="are for `engine='jinja'`"):
        Templates("app/sql", filters={"upper": str.upper})
...
def test_a_filter_that_has_to_be_awaited_is_refused() -> None:
    ...
    with pytest.raises(AsyncFilterError, match="rates"):
        Templates("app/sql", engine="jinja", globals={"rates": money})
    with pytest.raises(AsyncFilterError, match="money"):
        Templates(
            "app/sql", engine="jinja", filters={"money": Filter(money, bind=True)}
        )
