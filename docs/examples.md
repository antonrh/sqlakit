...
SQLModel classes are SQLAlchemy models, so a query works on them like any other.
Nothing here inherits from SQLAKit: the repository holds the database and hands
out queries.
...
attribute of a pydantic model wants an annotation, and `ClassVar` is how you say
this attribute is not a column.
